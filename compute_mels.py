...
if __name__ == "__main__":
    ...
    data = pd.read_csv('data/data_enriched_no_duplicates.csv')
    data['path'] = 'data/train_audio/' + data["filename"]
    data['mel_path'] = 'data/train_mels/' + data["filename"].apply(lambda s: s.replace('.ogg', '.npy'))
    ...
    unlabeled_dir = 'data/unlabeled_soundscapes/'
    paths = [unlabeled_dir+f for f in sorted(os.listdir(unlabeled_dir))]
    data_unlabeled = pd.DataFrame(paths, columns=['filepath'])
    data_unlabeled['filename'] = data_unlabeled.filepath.map(lambda x: x.split('/')[-1])
    data_unlabeled['mel_path'] = 'data/unlabeled_mels/' + data_unlabeled["filename"].apply(lambda s: s.replace('.ogg', '.npy'))
